...
# ╔═╡ a8b36337-b276-4eed-a850-2b39dacbc018
...
# ╔═╡ 2ee4afc9-ea91-44d0-8a95-6beb105debd2
function plot_surface(f;normalize=false,
	xg=range(-15,5,50),yg=range(-10,10,30),kwargs...)
	if normalize
		zg = f.(xg',yg)
		mx = maximum(abs,zg)
		levels = -0.9:0.1:0.9
...
begin
	...
	if show_green
		plot_surface((x,y)->(kelvin(SA[x,y,0],SA[4,4,Zp],Fn=Fnp)+kelvin(SA[x,y,0],SA[-4,0,Zp],Fn=Fnp)),normalize=true,c=:delta,
			colorbartitle="G/max(G)",title="Point source at Z=$Zp, Fn=$Fnp")
	else
		plot_surface((x,y)->derivative(x->(kelvin(SA[x,y,0],SA[0,4,Zp],Fn=Fnp)+kelvin(SA[x,y,0],SA[0,-4,Zp],Fn=Fnp)),x),
	...
	plot!([0,-15],[4,10],c=:black,ls=:dot,label=nothing)
	plot!([0,-15],[4,-2],c=:black,ls=:dot,label=nothing)
	plot!([0,-15],[-4,-10],c=:red,ls=:dot,label=nothing)
	plot!([0,-15],[-4,2],c=:red,ls=:dot,label=nothing)
end
